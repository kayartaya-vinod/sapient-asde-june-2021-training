...
@Slf4j
class VehicleReviewControllerPostRequestTests {
    ...
    void setup() throws ServiceException, RestClientException, JsonProcessingException {
        ...
        Mockito.when(service.saveReview(validUserId , validVehicleId , review)).thenReturn(feedback);
    }

    @Test
    void shouldGiveStatusOkWhenReviewIsUpdated() throws JsonProcessingException, Exception {
        ...
        .content(om.writeValueAsString(newFeedback))).andExpect(status().isOk());
    }

    @Test
    void shouldNotUpdateReviewWithoutToken() throws JsonProcessingException, Exception {
        String url = baseUrl + validVehicleId + "/reviews";
        log.info("URL = {}" , url);
        mockMvc.perform(post(url).contentType(MediaType.APPLICATION_JSON)
        .content(om.writeValueAsString(newFeedback))).andExpect(status().isUnauthorized());
    }
}
